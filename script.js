document.addEventListener("DOMContentLoaded", () => {
	...
	const totalStickyHeight = window.innerHeight * 3;
			...
			.map((part) => {
				if (part.trim() === "") {
					...
					return part
						...
						.map(
							(char) =>
								`<span style="opacity: 0; display: inline-block;">${char}</span>` //no olvidar que empieza con opacity 0
						)
						.join("");
				}
			})
	...
	//flicker animation
	function flickerAnimation(targets, toOpacity, staggerAmount) {
		gsap.to(targets, {
			opacity: toOpacity,
			duration: 0.09,
			stagger: {
	...
	ScrollTrigger.create({
		...
		onEnter: () => flickerAnimation(".intro-col p span", 1, 0.3),
		onLeave: () => flickerAnimation(".intro-col p span", 0, 0.9),
		onEnterBack: () => flickerAnimation(".intro-col p span", 1, 0.3),
		onLeaveBack: () => flickerAnimation(".intro-col p span", 0, 0.9),
	});
